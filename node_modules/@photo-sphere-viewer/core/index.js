...
/*!
 * PhotoSphereViewer 5.0.1
 * @copyright 2014-2015 Jérémy Heleine
 * @copyright 2022 Damien "Mistic" Sorel
...
"use strict";
  ...
  // src/utils/psv.ts
  function positionCompat(position) {
    return {
      ...position,
      get longitude() {
        logWarn("longitude is deprecated, use yaw instead");
        return this.yaw;
      },
      get latitude() {
  ...
  function parseAngle(angle, zeroCenter = false, halfCircle = zeroCenter) {
    let parsed;
  ...
  var FullscreenButton = class extends AbstractButton {
    constructor(navbar) {
      super(navbar, {
        ...
        collapsable: false,
        tabbable: true,
        icon: fullscreen_in_default,
        iconActive: fullscreen_out_default
      });
...
<div class="psv-panel-menu psv-panel-menu--stripped">
  <h1 class="psv-panel-menu-title">${menu_default} ${title}</h1>
...
`;
  ...
  function getIcon(value) {
    ...
    return arrow_default.replace("rotate(0", `rotate(${angle}`);
  }
  ...
  var ZoomInButton = class extends AbstractZoomButton {
    constructor(navbar) {
      super(navbar, 0 /* IN */, zoom_in_default);
    }
  };
  ...
  var ZoomOutButton = class extends AbstractZoomButton {
    constructor(navbar) {
      super(navbar, 1 /* OUT */, zoom_out_default);
    }
  };
  ...
  var DataHelper = class extends AbstractService {
    ...
    speedToDuration(value, angle) {
      if (typeof value !== "number") {
        ...
        return angle / Math.abs(speed) * 1e3;
      } else {
    ...
    cleanPosition(position) {
      ...
      if ("longitude" in position && "latitude" in position) {
        ...
        return this.cleanPosition({ yaw: position["longitude"], pitch: position["latitude"] });
      }
  ...
  var EventsHandler = class extends AbstractService {
    ...
    __onMouseUp(evt) {
      if (this.data.mousedown || this.data.step === MOVING) {
        this.__stopMove(evt.clientX, evt.clientY, evt.target, evt.button === 2);
      }
    }
    __onMouseMove(evt) {
      if (this.config.mousemove && (this.data.mousedown || this.data.step === MOVING)) {
        evt.preventDefault();
    ...
    __onTouchMove(evt) {
      ...
      if (evt.touches.length === 1) {
        if (this.config.touchmoveTwoFingers) {
          if (this.data.mousedown && !this.data.twofingersTimeout) {
        ...
        } else if (this.data.mousedown || this.data.step === MOVING) {
          evt.preventDefault();
          const touch = evt.touches[0];
    ...
    __doMove(clientX, clientY) {
      if (this.data.mousedown && (Math.abs(clientX - this.data.startMouseX) >= this.moveThreshold || Math.abs(clientY - this.data.startMouseY) >= this.moveThreshold)) {
        this.viewer.stopAll();
        this.__resetMove();
        this.data.step = MOVING;
